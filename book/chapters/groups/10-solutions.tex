...
&\overline8^{-1}=\overline2,
&&\overline{11}^{-1}=\overline{11},
&&\overline{13}^{-1}=\overline7,
...
반대로 $d=\gcd(m,n)>1$이면 $\lcm(m,n)=mn/d<mn$이다. $x=\lcm(m,n)$은 $m,n$의 공배수이므로 $\Phi(\overline x)=0$이지만 $\Z/mn\Z$에서 $\overline x\ne\overline0$이다. 따라서 $\Phi$는 단사가 아니다.
\end{solution}
