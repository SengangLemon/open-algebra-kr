...
$n\ge2$라 하자. 순열의 짝홀성을 기록하는 부호 준동형
\[
  \operatorname{sgn}:S_n\to\{\pm1\}
\]
은 전사이고 그 핵은 교대군 $A_n$이다. 따라서
\[
...
$N\trianglelefteq G$, $H\trianglelefteq G$, $N\subseteq H$이면 $H/N\trianglelefteq G/N$이고
\[
  (G/N)/(H/N)\cong G/H.
\]
\end{theorem}
...
\[
  \ker\Phi=H/N.
\]
따라서 $H/N$은 $G/N$의 정규부분군이고, 제1동형정리를 적용하면 원하는 동형을 얻는다.
\end{proof}

\begin{pitfall}
$N\trianglelefteq H$와 $H\trianglelefteq G$만으로 $N\trianglelefteq G$가 따라오지는 않는다. 예를 들어 $G=A_4$, $H=V_4$라 하고
\[
  N=\langle(12)(34)\rangle
\]
로 두면 $N\trianglelefteq H$이고 $H\trianglelefteq G$이지만 $N$은 $G$에서 정규가 아니다. 따라서 제3동형정리에서는 $N$과 $H$가 모두 $G$에서 정규라는 가정이 필요하다.
\end{pitfall}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item $N\trianglelefteq G$, $H\trianglelefteq G$, $N\subseteq H$에서 제3동형정리를 ``정보를 단계적으로 지우는 과정''으로 설명하라.
