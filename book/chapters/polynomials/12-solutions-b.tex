...
\begin{solution}
(a) $p=7$을 택한다. 최고차항계수 $1$은 $7$로 나누어지지 않고, 나머지 계수 $14,0,0,21,0,0,7$은 모두 $7$로 나누어진다. 상수항 $7$은 $49$로 나누어지지 않는다. 따라서 기약이다.

(b) $p=2$를 택한다. 최고차항계수 $3$은 짝수가 아니고, 나머지 계수 $10,0,20,0,50$은 모두 짝수이다. 상수항 $50$은 $4$로 나누어지지 않는다. 다항식의 내용은 $1$이므로 아이젠슈타인 판정법에 따라 기약이다.
\end{solution}
...
\end{array}
\]
차수 $2,3$에서는 $0,1$이 근이 아닌지를 검사하면 된다. 차수 $4$에서는 근이 없다는 조건과 유일한 기약 이차다항식 $X^2+X+1$로 나누어지지 않는 조건을 함께 검사한다.
\end{solution}
...
(c) $\alpha=\sqrt[3]{2}$라 하면 이 체는 실수체 $\Q(\alpha)$와 동형이다. 그러나 복소수에서 세 근은
\[
 \alpha,\qquad \omega\alpha,\qquad \omega^2\alpha,
\]
여기서 $\omega=e^{2\pi i/3}$이다. $\Q(\alpha)$는 실수만 포함하므로 두 비실근을 포함하지 않는다.
