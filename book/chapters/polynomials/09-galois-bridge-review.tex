...
\begin{keyidea}
기약다항식 $f$는 하나의 근 $\alpha$를 첨가한 단순확장
\[
 K(\alpha)\cong K[X]/(f)
\]
을 만든다. 이때 $f$는 $\alpha$가 만족하는 가장 낮은 차수의 관계가 된다. 이후에는 $f$의 나머지 근을 첨가하고, 그 근들을 보존하는 대칭을 연구한다.
\end{keyidea}
...
\]
로 유일하게 표현된다. 그러나 $L$은 $f$의 세 근을 모두 포함하지는 않는다. 나머지 복소근을 포함하려면 더 큰 분해체가 필요하다.
\end{example}
...
\begin{enumerate}
  ...
  \item $K[X]$는 유클리드 정역이므로 최대공약수, 베주 항등식, 유일인수분해가 가능하다.
  \item $f(a)=0$일 필요충분조건은 $(X-a)\mid f$이다.
  \item 중근은 $f$와 $f'$의 공통근이며, 제곱인수가 없을 조건은 $\gcd(f,f')=1$이다.
  \item 차수 $2$ 또는 $3$에서는 근의 존재가 가약성과 동치이다.
  \item $f$가 기약일 필요충분조건은 $K[X]/(f)$가 체인 것이다.
  \item 원시다항식의 $R[X]$에서의 기약성과 $Q(R)[X]$에서의 기약성은 동치이다.
...
\begin{description}[style=nextline]
  ...
  \item[Q] $\overline f\in K[X]/(m)$가 단위원일 조건은?\\
  \textbf{A} $\gcd(f,m)=1$.

  \item[Q] 차수 $2$ 또는 $3$인 다항식의 기약성 검사법은?\\
  \textbf{A} 계수체 안에 근이 있는지 검사한다.

  \item[Q] $K[X]/(f)$가 체일 조건은?\\
