...
환 준동형 $\varphi:R\to S$와 $s\in S$가 주어지면
\[
  \Phi:R[X]\to S,\qquad
...
\]
로 강제되므로 유일하다. 반대로 위 공식은 덧셈을 보존하고, 코시 곱을 전개하면 $\Phi(fg)=\Phi(f)\Phi(g)$도 성립한다.
\end{proof}

$R\subseteq S$일 때 이 준동형을 $s$에서의 평가 준동형이라 하고 $\operatorname{ev}_s(f)=f(s)$로 쓴다.

\begin{keyidea}
다항식환은 계수환 $R$과 자유롭게 선택할 수 있는 한 원소 $X$를 가진 가장 일반적인 환이다. 어떤 환에서 $X$ 대신 $s$를 선택하면 유일한 대입 준동형이 생긴다.
\end{keyidea}
