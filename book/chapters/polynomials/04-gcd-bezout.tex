...
\begin{definition}
$f,g\in K[X]$가 동시에 영다항식은 아니라고 하자. 일계수다항식 $d$가
...
일계수 조건은 단위원배의 모호성을 제거한다. $K[X]$의 단위원은 $K^\times$이므로 최대공약수는 상수배를 제외하면 유일하다. 이 정의에 따라 $g\ne0$이면 $\gcd(0,g)$는 $g$를 일계수로 만든 다항식이다.

\begin{theorem}[유클리드 알고리즘]
$f,g\in K[X]$, $g\ne0$라 하자. 나머지가 $0$이 될 때까지 연속적으로 나눗셈을 수행하면 마지막 영이 아닌 나머지를 일계수로 만든 다항식이 $\gcd(f,g)$이다.
\end{theorem}

\begin{proofidea}
각 나눗셈 $a=qb+r$에서 $a,b$의 공약수와 $b,r$의 공약수는 같다. 영이 아닌 나머지들의 차수가 엄격히 감소하므로 과정은 유한 번 만에 끝난다.
\end{proofidea}
...
\end{aligned}
\]
와 같이 쓰자. $a=qb+r$이면 $a,b$의 공약수는 $b,r$의 공약수와 정확히 같다. 이 사실을 각 줄에 반복 적용하면 $f,g$의 공약수들은 $r_n,0$의 공약수들과 같다. 따라서 $r_n$은 단위원배를 제외하고 $f,g$의 최대공약수이며, 최고차항계수로 나누면 유일한 일계수 최대공약수를 얻는다.
\end{proof}

\begin{theorem}[베주 항등식]
$f,g\in K[X]$가 동시에 영다항식은 아니라고 하자. 그러면 어떤 $a,b\in K[X]$가 존재하여
\[
  \gcd(f,g)=af+bg
\]
가 성립한다. 특히
...
유클리드 알고리즘의 각 식을 마지막 식부터 거꾸로 대입하면 위의 $a,b$를 실제로 계산할 수 있다. 이를 확장 유클리드 알고리즘이라 한다.
...
\begin{proof}
$af+bm=1$이면 몫환에서 $\overline a\,\overline f=\overline1$이다. 반대로 $\overline f$가 역원을 가지면 어떤 $a\in K[X]$에 대해 $af-1\in(m)$이므로 $af+bm=1$인 $b\in K[X]$가 존재한다. 따라서 $f,m$의 최대공약수는 $1$이다.
\end{proof}
