...
\[
 \prod_{a\in\F_q}(X-a)
\]
도 같은 $q$개 근을 갖는 차수 $q$의 일계수다항식이다. 두 다항식의 차는 최고차항이 소거되어 차수가 $q$보다 작으면서 $q$개의 서로 다른 근을 가지므로 영다항식이다. 따라서
\[
...
\begin{solution}
$f=gh$가 $\Q[X]$에서의 비자명한 인수분해라고 하자. 각 인수의 분모를 없앤 뒤 계수들의 최대공약수도 제거하면
\[
  g=cg_0,
  \qquad
  h=dh_0
\]
로 쓸 수 있다. 여기서 $c,d\in\Q^\times$이고 $g_0,h_0\in\Z[X]$는 원시다항식이며 둘 다 양의 차수이다. 따라서
\[
  f=cd\,g_0h_0.
\]
가우스 보조정리에 의해 $P=g_0h_0$는 원시이다.

이제 $cd=a/b$를 서로소인 정수 $a,b$와 $b>0$으로 쓰자. $f=(a/b)P$의 모든 계수가 정수이므로 $b$는 $a$와 $P$의 각 계수의 곱을 나눈다. $\gcd(a,b)=1$이므로 $b$의 모든 소인수는 $P$의 모든 계수를 나누어야 한다. 그러나 $P$는 원시이므로 $b=1$이다. 따라서 $cd=a\in\Z$이다. 더 나아가 $f=aP$도 원시이므로 $|a|=1$이어야 한다. 부호를 한 인수에 흡수하면
\[
  f=(\pm g_0)h_0
\]
