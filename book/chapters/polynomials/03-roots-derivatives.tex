...
\begin{proof}
$f$의 분해체 $L$에서 생각하자. $\gcd(f,f')\ne1$이면 두 다항식은 양의 차수인 공통인수를 갖고, 그 공통인수는 $L$에서 어떤 근 $\alpha$를 갖는다. 그러면 $f(\alpha)=f'(\alpha)=0$이므로 $\alpha$는 중근이다. 반대로 $f$가 어떤 확대체에서 중근 $\alpha$를 가지면 $X-\alpha$가 그 확대체의 다항식환에서 $f$와 $f'$를 모두 나눈다. 따라서 $K[X]$에서의 $\gcd(f,f')$도 상수가 될 수 없다.
\end{proof}
