...
$e=[\F_q(\alpha):\F_q]$라 하자. $m_{\alpha,\F_q}\mid g$이므로 $e\le d$이다. 한편 $\F_q(\alpha)=\F_{q^e}$이고 $\alpha\in\F_{q^d}$이므로
\[
  \F_{q^e}\subseteq\F_{q^d},
\]
따라서 $e\mid d$이다. 반대로 $\alpha\in\F_{q^e}$이므로 $\alpha^{q^e}=\alpha$이고, $d$의 최소성에서 $d\mid e$이다. 결국 $d=e$이고, $g$와 최소다항식은 같은 차수의 일계수 다항식이므로 서로 같다.
\end{proof}
...
\begin{proof}
$\F_{q^n}^\times$의 생성원 $\gamma$를 택하자. 만약 $\gamma$가 진부분체 $\F_{q^d}$에 속한다면 $d<n$이고 $d\mid n$이며, $\gamma$의 위수는 $q^d-1$을 나누어야 한다. 그러나 $\gamma$의 위수는 $q^n-1$이므로 모순이다. 따라서
\[
  \F_q(\gamma)=\F_{q^n}
\]
이고 $\gamma$의 최소다항식 차수는 $n$이다.
\end{proof}
