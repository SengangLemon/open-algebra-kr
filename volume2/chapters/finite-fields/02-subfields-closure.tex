...
\begin{learninggoal}
고정된 대수적 폐포 안에서 유한체 사이의 포함관계를 분류한다. 교집합과 합성체를 계산하고, 유한체의 모든 대수적 확장이 정규·분리임을 증명한다.
\end{learninggoal}
...
\begin{theorem}
유한체 $\F_q$의 모든 대수적 확장은 정규확장이면서 분리확장이다. 특히 모든 유한체는 완전체이다.
\end{theorem}
...
\[
  X^{q^n}-X
\]
의 $\F_q$ 위 분해체이다. 이 다항식의 도함수는 $-1$이므로 분리다항식이다. 따라서 $E/\F_q$는 정규이면서 분리이다.

이제 $L/\F_q$가 임의의 대수적 확장이라 하자. 각 $\alpha\in L$는 유한 부분확장 $\F_q(\alpha)$에 들어간다. 위의 논증에 의해 $\alpha$의 최소다항식은 그 유한체 안에서 완전히 분해되고 중근이 없다. 따라서 $L/\F_q$ 전체가 정규이면서 분리이다. 모든 대수적 확장이 분리이므로 $\F_q$는 완전체이다.
\end{proof}

\begin{remark}
정규확장의 정의와 동치조건은 다음 장에서 체계적으로 다룬다. 여기서는 유한체의 유한확장이 분리다항식 $X^{q^n}-X$의 분해체라는 사실이 정규성과 분리성을 동시에 준다는 점만 사용한다.
\end{remark}
