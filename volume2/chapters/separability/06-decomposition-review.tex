...
\begin{description}[style=nextline,leftmargin=0pt]
  \item[분리다항식의 계산 판정은?] $\gcd(f,f')=1$.
  \item[기약다항식이 비분리일 수 있는 표수는?] 양의 표수 $p$뿐이다.
  \item[$[L:K]_{\mathrm s}$의 의미는?] $K$를 고정하는 $L\to\overline K$ 매장의 수.
  \item[유한확장이 분리일 조건은?] $[L:K]_{\mathrm s}=[L:K]$.
  \item[완전체의 양의 표수 판정은?] 프로베니우스 $x\mapsto x^p$의 전사성.
  \item[순수비분리 원소의 판정은?] 어떤 $r$에 대해 $\alpha^{p^r}\in K$.
  \item[분리폐포 $K_{\mathrm s}$는?] $L$ 안에서 $K$ 위 분리적인 모든 원소의 체.
\end{description}
