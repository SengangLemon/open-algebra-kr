...
\[
  ...
  =\Res(f,g_1)\Res(f,g_2).
\]
더 일반적으로 $f_1,f_2,g$의 형식적 차수를 각각 $m_1,m_2,n$으로 정하면
\[
  \Res_{m_1+m_2,n}(f_1f_2,g)
  =\Res_{m_1,n}(f_1,g)\Res_{m_2,n}(f_2,g).
\]
마찬가지로 $f,g_1,g_2$의 형식적 차수가 $m,n_1,n_2$이면
\[
  \Res_{m,n_1+n_2}(f,g_1g_2)
  =\Res_{m,n_1}(f,g_1)\Res_{m,n_2}(f,g_2).
\]
특히 계수환이 정역이면 실제 차수에 대한 익숙한 곱셈공식을 얻는다.
\end{corollary}
...
일반 형식적 차수 공식을 보이기 위해 모든 계수를 독립인 변수로 둔 보편 다항식환 $U$에서 먼저 계산한다. 이 환은 정역이다. $f$의 최고차계수를 $a$라 하고 $U[a^{-1}]$로 국소화하면
\[
  \widetilde f=a^{-1}f
\]
는 일계수이다. 스케일링 성질과 방금 증명한 일계수의 곱셈성을 적용하면
\[
\begin{aligned}
  \Res(f,g_1g_2)
  &=a^{n_1+n_2}\Res(\widetilde f,g_1g_2)\\
  &=a^{n_1+n_2}\Res(\widetilde f,g_1)
    \Res(\widetilde f,g_2)\\
  &=\Res(f,g_1)\Res(f,g_2)
\end{aligned}
\]
가 $U[a^{-1}]$에서 성립한다. $U\hookrightarrow U[a^{-1}]$가 단사이고 양변은 보편 계수들의 다항식이므로 같은 항등식이 이미 $U$에서 성립한다. 임의의 $R$로 특수화하면 두 번째 변수에 관한 형식적 차수 공식을 얻는다. 첫 번째 변수에 관한 공식은
\[
  \Res_{m,n}(f,g)=(-1)^{mn}\Res_{n,m}(g,f)
\]
를 함께 사용하면 따른다.
\end{proof}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item $K$가 체이고 $f,g\in K[X]$가 영이 아닌 서로소다항식이면 실베스터 사상이 왜 동형인지 설명하라.
