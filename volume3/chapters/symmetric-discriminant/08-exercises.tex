...
\begin{exercise}[*]
가환환 $A$와 일계수 $d$차다항식 $h\in A[X]$, $d\ge1$에 대해
\[
...
\begin{exercise}[*]
형식적 차수가 각각 $m,n$인 $f,g\in R[X]$에 대해 실베스터 사상의 수반행렬을 사용하여
\[
  \Res_{m,n}(f,g)=pf+qg,
  \qquad
...
\[
  f_{p,q}=X^3+pX+q
\]
라 하자. $f_{p,q}$가 중근을 갖는 정수쌍 $(p,q)$의 조건을 구하고, 가능한 중근 $r\in\Z$를 사용하여
\[
  p=-3r^2,
  \qquad q=2r^3
\]
...
\begin{exercise}
일계수 기약 삼차다항식 $f\in\Q[X]$의 판별식이 정수의 제곱이면 분해체가 한 근을 첨가한 삼차체와 같음을 증명하라. 판별식이 제곱이 아니면 분해체 차수가 $6$임을 설명하라.
\end{exercise}

\begin{exercise}
$f\in R[X]$가 일계수 $n$차다항식이고 $u\in R^\times$, $c\in R$라 하자. 다항식
\[
  g(X)=u^{-n}f(uX+c)
\]
에 대해 근 공식을 이용하여
...
\begin{exercise}
$K$가 체이고 $m,n\ge1$, $a,b\in K$라 하자. $f=X^m-a$, $g=X^n-b$에 대해 $f$의 근들을 이용하여 $\Res(f,g)$를 곱으로 표현하라. $m=n$인 경우 공통근이 존재할 조건을 해석하라.
\end{exercise}

\begin{exercise}[*]
가환환 $R$ 위에서 형식적 차수가 모두 $2$인 두 다항식
\[
  f=a_0X^2+a_1X+a_2,
  \qquad
  g=b_0X^2+b_1X+b_2
