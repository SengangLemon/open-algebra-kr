...
\begin{exercise}[*]
정$m$각형과 정$n$각형이 모두 작도가능하고 $d=\gcd(m,n)\ge3$이라 하자. 정$d$각형과 정$\operatorname{lcm}(m,n)$각형도 작도가능함을 두 방법으로 증명하라.
\begin{enumerate}[label=(\alph*)]
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item 적절한 중간체들을 택하여 $\Q$에서 $E$까지 차수 $2$인 확장들의 탑을 만들 수 있다.
\end{enumerate}
\end{exercise}
