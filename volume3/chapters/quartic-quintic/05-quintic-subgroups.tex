...
\]
를 만족하므로 $n_5=6$이다. 서로 다른 위수 $5$ 부분군은 항등원 외에는 만나지 않으므로, 이 여섯 부분군은 $S_5$의 $24$개 $5$-순환을 모두 포함한다.

이제 $G$가 $A_5$ 전체를 포함함을 직접 확인하자. 오른쪽 순열을 먼저 작용시키는 관례에서
\[
  (1\,3\,4\,2\,5)(1\,4\,3\,5\,2)
  =(1\,2\,3)
\]
이고 왼쪽의 두 인수는 모두 $5$-순환이므로 $G$에 속한다. 이 항등식을 $S_5$의 임의의 순열로 켤레시키면 모든 $3$-순환이 두 $5$-순환의 곱으로 표현된다. $G$는 모든 $5$-순환을 포함하므로 모든 $3$-순환을 포함한다. 교대군은 $3$-순환들로 생성되므로
\[
