...
\begin{proof}
곱셈사상은 군준동형이다. $\xi\eta=1$이면
\[
  \xi=\eta^{-1}\in\mu_m\cap\mu_n.
\]
이 원소의 위수는 $m$과 $n$을 모두 나누므로 $\gcd(m,n)=1$에서 $\xi=\eta=1$이다. 따라서 사상은 단사이다. 정의역과 공역은 모두 위수 $mn$이므로 전단사이다.

$\xi$와 $\eta$가 각각 원시근이면 그 위수는 서로소인 $m,n$이다. 따라서 $\xi\eta$의 위수는 $mn$이고 원시 $mn$차 단위근이다.
\end{proof}
