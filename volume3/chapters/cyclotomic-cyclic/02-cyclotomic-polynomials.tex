...
\begin{proof}
원시 단위근의 역수도 원시 단위근이다. 따라서 근들의 집합이 역수에 대해 닫혀 있고, 일계수다항식의 근을 역수로 바꾸어도 같은 다항식을 얻는다. 상수항은 근들의 곱에 부호를 붙인 값이며 영이 아닌 정수이자 절댓값 $1$인 복소수이므로 $1$이다.
\end{proof}

\subsection{$\Q$ 위에서의 기약성}
...
\[
  \deg f\ge\varphi(n).
\]
반대로 $f$의 근은 $X^n-1$의 근이고 $f$는 $\Phi_n$을 나누므로 $\deg f\le\varphi(n)$이다. 따라서 $f=\Phi_n$이고 차수는 $\varphi(n)$이다. 마지막으로 $f$가 일계수 정수다항식이므로 $\Phi_n\in\Z[X]$이다.
\end{proof}
